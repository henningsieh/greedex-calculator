...
| --- | --- | --- | --- | --- |
| Calculator | `pnpm turbo run build --filter=@greendex/calculator` | `pnpm turbo run start --filter=@greendex/calculator` | `greendex.apps.sieh.org:3000`, `socket.greendex.apps.sieh.org:4000` | `/api/rpc/health` on `3000` |
| Documentation | `pnpm turbo run build --filter=@greendex/documentation` | `pnpm turbo run start --filter=@greendex/documentation` | `docs.greendex.apps.sieh.org:3001` | `/en` on `3001` |
...
Preserve `"env": ["*"]` on Turbo `build` and `start`. Calculator build and Coolify must both use `NODE_ENV=production`; a copied `development` build variable causes invalid Next.js builds. `NEXT_PUBLIC_*` preview values are build-time values and must name the PR hosts. Calculator preview `DATABASE_URL` must use the preview database UUID hostname, never the host IP or public port. Authentication and mail settings may mirror development unless the task says otherwise.

## Sequential preview deployment runbook

**Mutex:** exactly one Greendex deployment may be `queued` or `in_progress` across both applications. A code push can trigger every preview-enabled app concurrently even when ordinary auto-deploy is disabled.

1. **Quiesce.** Disable `is_preview_deployments_enabled` and `is_auto_deploy_enabled` on both apps before pushing a PR commit. List recent deployments for both app UUIDs and cancel every `queued` or `in_progress` record. Completion: the global active set is empty.
2. **Publish.** Run local format, lint, type-check, tests, and production builds; commit and push. Completion: the PR head SHA is the tested SHA and no Coolify deployment was created.
3. **Select one app.** Enable preview deployments only for the app being deployed; leave the other app disabled. Validate its command, ports, health check, preview URL template, and preview environment. Completion: one app enabled, one disabled, active set empty.
4. **Trigger.** Request only that application UUID with the PR number. Record the returned `deployment_uuid`. Completion: the global active set contains only that UUID.
...
Set `COOLIFY_API_URL`, `COOLIFY_TOKEN`, `PR_NUMBER`, and `APPLICATION_UUID` outside the repository. Never use the token reserved for another host.
...
# List an application's records before acquiring the mutex.
curl -fsS \
  -H "Authorization: Bearer $COOLIFY_TOKEN" \
  "$COOLIFY_API_URL/deployments/applications/$APPLICATION_UUID?take=10"
