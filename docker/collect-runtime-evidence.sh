...
readonly image_digest="$(docker image inspect --format '{{join .RepoDigests "\n"}}' "$container_image_id" | awk 'NF { print; exit }')"

if [[ "$container_image_id" != "$inspected_image_id" ]]; then
  echo "The selected container image ID does not match the inspected image." >&2
  exit 1
fi

if [[ -z "$image_digest" || "$image_digest" != *@sha256:* ]]; then
  echo "The selected image does not have an immutable repository digest." >&2
  exit 1
fi
